...
# --- NOWE DANE Z TWOICH SCREENÓW (OSTATNIE MECZE) ---
@st.cache_data
def load_recent_stats():
    # Dane wyciągnięte ze screenów: GF/GA z tabeli "Form" (6 meczów), xG/xGA z tabeli xG (5 meczów)
    # Wyjątki: Frankfurt (4 mecze xG)
    return {
        'Bayern Munich': {'GF': 20/6, 'GA': 8/6, 'xG': 16.10/5, 'xGA': 8.36/5, 'm_g': 6, 'm_xg': 5},
        'Borussia Dortmund': {'GF': 15/6, 'GA': 9/6, 'xG': 8.22/5, 'xGA': 10.06/5, 'm_g': 6, 'm_xg': 5},
        'Augsburg': {'GF': 9/6, 'GA': 7/6, 'xG': 10.02/5, 'xGA': 8.57/5, 'm_g': 6, 'm_xg': 5},
        'VfB Stuttgart': {'GF': 14/6, 'GA': 8/6, 'xG': 9.62/5, 'xGA': 9.40/5, 'm_g': 6, 'm_xg': 5},
        'RB Leipzig': {'GF': 11/6, 'GA': 9/6, 'xG': 13.56/5, 'xGA': 7.65/5, 'm_g': 6, 'm_xg': 5},
        'Bayer Leverkusen': {'GF': 10/6, 'GA': 6/6, 'xG': 11.02/5, 'xGA': 7.04/5, 'm_g': 6, 'm_xg': 5},
        'Hoffenheim': {'GF': 13/6, 'GA': 11/6, 'xG': 11.76/5, 'xGA': 10.16/5, 'm_g': 6, 'm_xg': 5},
        'Hamburger SV': {'GF': 9/6, 'GA': 7/6, 'xG': 8.49/5, 'xGA': 12.42/5, 'm_g': 6, 'm_xg': 5},
        'St. Pauli': {'GF': 7/6, 'GA': 9/6, 'xG': 4.00/5, 'xGA': 7.81/5, 'm_g': 6, 'm_xg': 5},
        'Mainz 05': {'GF': 8/6, 'GA': 9/6, 'xG': 10.93/5, 'xGA': 6.86/5, 'm_g': 6, 'm_xg': 5},
        'Eintracht Frankfurt': {'GF': 10/6, 'GA': 10/6, 'xG': 6.40/4, 'xGA': 4.98/4, 'm_g': 6, 'm_xg': 4},
        'Freiburg': {'GF': 6/6, 'GA': 10/6, 'xG': 5.63/5, 'xGA': 7.91/5, 'm_g': 6, 'm_xg': 5},
        'Werder Bremen': {'GF': 4/6, 'GA': 9/6, 'xG': 9.23/5, 'xGA': 5.34/5, 'm_g': 6, 'm_xg': 5},
        'Borussia M.Gladbach': {'GF': 5/6, 'GA': 11/6, 'xG': 6.62/5, 'xGA': 7.28/5, 'm_g': 6, 'm_xg': 5},
        'Union Berlin': {'GF': 5/6, 'GA': 11/6, 'xG': 4.21/5, 'xGA': 8.21/5, 'm_g': 6, 'm_xg': 5},
        'FC Cologne': {'GF': 6/6, 'GA': 11/6, 'xG': 4.83/5, 'xGA': 10.96/5, 'm_g': 6, 'm_xg': 5},
        'Wolfsburg': {'GF': 6/6, 'GA': 14/6, 'xG': 8.22/5, 'xGA': 10.54/5, 'm_g': 6, 'm_xg': 5},
        'FC Heidenheim': {'GF': 7/6, 'GA': 15/6, 'xG': 6.64/5, 'xGA': 11.98/5, 'm_g': 6, 'm_xg': 5}
    }

# --- FUNKCJA OBLICZANIA BONUSU ---
def calculate_team_bonus(team_name, base_row, is_home):
    recent_stats = load_recent_stats()
    if team_name not in recent_stats:
        return 0.0, 0.0, None # Brak danych dla PL/LL w tym momencie
    
    r = recent_stats[team_name]
    # Wybór bazy (Sezon)
    b_xgf = base_row['HxG_F'] if is_home else base_row['AxG_F']
    b_xga = base_row['HxG_A'] if is_home else base_row['AxG_A']
    
    # 1. Atak
    trend_atk = (r['xG'] - b_xgf) / b_xgf
    skut_atk = (r['GF'] - r['xG']) / r['xG']
    raw_atk = (trend_atk * 0.7) + (skut_atk * 0.3)
    final_atk = raw_atk * 0.25
    
    # 2. Obrona
    trend_def = (b_xga - r['xGA']) / b_xga
    skut_def = (r['xGA'] - r['GA']) / r['xGA']
    raw_def = (trend_def * 0.7) + (skut_def * 0.3)
    final_def = raw_def * 0.25
    
    return final_atk, final_def, {"r": r, "b_xgf": b_xgf, "b_xga": b_xga, "t_atk": trend_atk, "s_atk": skut_atk, "t_def": trend_def, "s_def": skut_def}
...
def load_la_liga():
    data = {
        ...
        'TxG_A': [1.89, 1.41, 1.47, 1.61, 1.38, 1.42, 1.63, 1.57, 1.57, 1.57, 1.49, 1.63, 1.95, 1.91, 2.02, 1.53, 2.08, 2.25, 1.93, 2.15],
...
def render_league_ui(df, league_name):
    ...
    # --- NOWY MODUŁ GRAFICZNY I OBLICZENIOWY: BONUS ---
    st.divider()
    st.markdown("<h3 style='text-align: center;'>🚀 Analiza Trendu i Bonusów Formy</h3>", unsafe_allow_html=True)
    
    h_atk_bon, h_def_bon, h_calc = calculate_team_bonus(h_team, h_row, True)
    a_atk_bon, a_def_bon, a_calc = calculate_team_bonus(a_team, a_row, False)
    
    def show_bonus_ui(team, calc, title):
        if not calc: 
            st.info(f"Brak dodatkowych danych trendu dla {team}")
            return 0, 0
        
        st.markdown(f"#### Dane Wejściowe dla {team}")
        data_table = {
            "Parametr": ["Gole Strzelone (GF)", "Gole Stracone (GA)", "xG (Kreacja)", "xGA (Dopuszczone)"],
            "Sezon (Baza)": [f"{h_row['H_GF'] if title=='Dom' else a_row['A_GF']:.2f}", 
                             f"{h_row['H_GA'] if title=='Dom' else a_row['A_GA']:.2f}", 
                             f"{calc['b_xgf']:.2f}", f"{calc['b_xga']:.2f}"],
            f"Ostatnie {calc['r']['m_g']} meczów": [f"{calc['r']['GF']:.2f}", f"{calc['r']['GA']:.2f}", f"{calc['r']['xG']:.2f}", f"{calc['r']['xGA']:.2f}"]
        }
        st.table(pd.DataFrame(data_table))
        
        col1, col2 = st.columns(2)
        with col1:
            st.write(f"**Bonus Ataku:**")
            st.write(f"* Trend Kreacji (70%): `{calc['t_atk']:.2%}`")
            st.write(f"* Skuteczność (30%): `{calc['s_atk']:.2%}`")
            st.write(f"**Wynik: Atak zmieniony o {h_atk_bon if title=='Dom' else a_atk_bon:.2%}**")
        with col2:
            st.write(f"**Bonus Obrony:**")
            st.write(f"* Trend Defensywny (70%): `{calc['t_def']:.2%}`")
            st.write(f"* Skuteczność Obrony (30%): `{calc['s_def']:.2%}`")
            st.write(f"**Wynik: Obrona zmieniona o {h_def_bon if title=='Dom' else a_def_bon:.2%}**")
        return h_atk_bon if title=='Dom' else a_atk_bon, h_def_bon if title=='Dom' else a_def_bon

    c_ui_h, c_ui_a = st.columns(2)
    with c_ui_h: hb_atk, hb_def = show_bonus_ui(h_team, h_calc, "Dom")
    with c_ui_a: ab_atk, ab_def = show_bonus_ui(a_team, a_calc, "Wyjazd")

    # --- OBLICZENIA POISSONA Z UWZGLĘDNIENIEM BONUSU ---
    ...
    # Dodanie bonusu do lambdy (bonus ataku podnosi, bonus obrony przeciwnika obniża)
    lambda_f = (h_atk_s * a_def_s * avg_h_gf) * (1 + h_total_mod + hb_atk - ab_def)
    mu_f = (a_atk_s * h_def_s * avg_a_gf) * (1 + a_total_mod + ab_atk - hb_def)
    ...
    with st.expander("🧮 Szczegółowa Ścieżka Obliczeniowa"):
        ...
        st.latex(rf"\lambda_{{final}} = \lambda_{{base}} \times (1 {h_total_mod + hb_atk - ab_def:+.2f}) = {lambda_f:.3f}")
        st.latex(rf"\mu_{{final}} = \mu_{{base}} \times (1 {a_total_mod + ab_atk - hb_def:+.2f}) = {mu_f:.3f}")
