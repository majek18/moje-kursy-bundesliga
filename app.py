...
for i, key in enumerate(['w_xg_dv', 'w_g_dv', 'w_xg_all', 'w_g_all']):
    if key not in st.session_state: st.session_state[key] = D_W[i]
...
# --- TABELA SIŁY ---
st.write("### 📊 Analiza Siły (Strength Factors)")
...
# --- NOWA MACIERZ (GREEN THEME) ---
with st.expander("⚽ Prawdopodobieństwo Wyniku (Dokładny Wynik)"):
    # Macierz ograniczona do 5x5 dla czytelności
    limit = 5
    m_plot = matrix[:limit, :limit]
    
    fig, ax = plt.subplots(figsize=(8, 5))
    # Paleta "YlGn" - od żółtego do soczystej zieleni
    sns.heatmap(m_plot, annot=True, fmt=".1%", cmap="YlGn", cbar=False, 
                linewidths=1, linecolor='#f0f2f6',
                xticklabels=range(limit), yticklabels=range(limit))
    
    plt.title(f"Szansa na wynik: {h_team} vs {a_team}", fontsize=12, pad=20)
    plt.xlabel(f"Gole {a_team}", fontsize=10)
    plt.ylabel(f"Gole {h_team}", fontsize=10)
    
    # Dodanie tła pasującego do Streamlit
